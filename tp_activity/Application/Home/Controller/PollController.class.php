...
class PollController extends Controller {
    ...
    public function draw(){
        ...
        $polldata = $polltime->field('opentime_start,opentime_end,status')->where('id=%d and publish_status=1',array(I('request.activeid')))->find();
        //判断活动是否在进行中
        ...
        //判断活动是否已过期
        if($polldata['opentime_start']>date('Y-m-d H:i:s',time()) || $polldata['opentime_end']<date('Y-m-d H:i:s',time())){
