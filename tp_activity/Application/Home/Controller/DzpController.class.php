...
//        $activeStatus = $redis->hgetall('activity_conf_'.I('get.activeid'));
        ...
        $activeStatus = $activeConf->field('status,opentime_start,opentime_end')->where('id=%d and publish_status=1',array(I('post.activeid')))->find();
        if($activeStatus['status']==1){
        ...
        if($activeStatus['opentime_start']>date('Y-m-d H:i:s',time()) || $activeStatus['opentime_end']<date('Y-m-d H:i:s',time())){
        ...
        //查看是否中奖
        $winRows=$activity->where('traffic_id="%s" and prize_level!=6 and publish_status=1',array(I('post.trafficid')))->count();
        if(!empty($winRows)){
...
//     $conf = $redis->hgetall('activity_conf_'.I('get.activeid'));
     ...
     $datames=$user->field('name,phone,address')->where('traffic_id="%s" and prize_level!=6 and activity_id=%d and publish_status=1',array(I('get.trafficid'),I('get.activeid')))->order('created_at desc')->find();
